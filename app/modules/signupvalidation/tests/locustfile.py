...
class SignupvalidationBehavior(TaskSet):
    ...
    def signup_invalido(self):
        ...
        response = self.client.post(
            "/signup",
            data={"email": " ", "password": fake.password(), "csrf_token": csrf_token},
        )
        if response.status_code == 200:
            assert (
                "This field is required" in response.text
...
def signup_con_email_repetido(self):
    ...
    csrf_token = get_csrf_token(
        response
    )  # Función que obtiene el CSRF token de la respuesta
    ...
    if response.status_code == 200:
        assert (
            "Email already in use" in response.text
