...
def test_forgot_password_flow(test_client):
    ...
    assert b"Check your email for the instructions to change your password" in response.data
...
def test_change_password_with_non_matching_passwords(test_client):
    ...
    assert b'id="submit_button" disabled' in response.data  # Verificar que el botón está deshabilitado
...
def test_login_with_old_password_failure(test_client):
    ...
    response = test_client.post('/login', data={'email': user_email, 'password': 'OldPassword123!'})

    # Verificar que el login falle
    assert response.status_code == 200
    assert b"Invalid credentials" in response.data  # Mensaje esperado en la pantalla de login
...
def test_login_with_new_password_(test_client):
    ...
    response = test_client.post('/login', data={'email': user_email, 'password': 'NewPassword123!'})
