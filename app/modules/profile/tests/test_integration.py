...
def test_client(test_client):
    ...
    with test_client.application.app_context():
        ...
        profile = UserProfile(
            user_id=user_test.id, name="Name", surname="Surname", is_verified=True
        )
...
def test_integration_profile_summary_access(test_client):
    ...
    ), "The profile summary page could not be accessed."
    assert (
        b"Summary" in response.data or b"Datasets" in response.data
...
def test_profile_edit_integration(test_client):
    ...
    assert (
        b"Profile updated" in response.data or b"successfully" in response.data
    ), "Profile update success message not found."
...
def test_profile_edit_integration_invalid_orcid(test_client):
    ...
    assert (
        response.status_code == 200
    ), "Profile update with invalid ORCID failed unexpectedly."
    assert (
        b"Invalid" in response.data or b"ORCID" in response.data
    ), "Expected validation error for ORCID not found."
...
def test_profile_edit_integration_long_name(test_client):
    ...
    assert (
        response.status_code == 200
    ), "Profile update with long name or surname failed unexpectedly."
    assert (
        b"too long" in response.data or b"validation" in response.data
    ), "Expected validation error for name or surname not found."
...
def test_user_datasets_access(test_client):
    ...
    ), "The expected content is not present on the datasets page"
