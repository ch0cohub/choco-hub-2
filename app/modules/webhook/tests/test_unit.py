...
def test_webhook_deploy_authorized(test_client):
    ...
    with test_client.application.app_context():
        ...
        with patch.object(
            WebhookService, "get_web_container", return_value="web_container_mock"
        ), patch.object(WebhookService, "execute_container_command"), patch.object(
            WebhookService, "log_deployment"
        ), patch.object(
            WebhookService, "restart_container"
        ):

            response = test_client.post(
                url_for(
                    "webhook.deploy", _external=True
                ),  # external=True para obtener la URL completa
...
def test_get_volume_name():
    ...
    mock_container.attrs = {
        "Mounts": [{"Destination": "/app", "Name": "volume_name_mock"}]
    }
...
def test_log_deployment():
    ...
    mock_container = Mock()
    with patch(
        "app.modules.webhook.services.WebhookService.execute_container_command"
    ) as mock_execute:
...
def test_restart_container():
    ...
    with patch("subprocess.Popen") as mock_popen:
        ...
        mock_popen.assert_called_once_with(
            ["/bin/sh", "/app/scripts/restart_container.sh", "test_container_id"]
        )
