...
def like_feature_model():
    ...
    review = FeatureModelReview.query.filter_by(
        feature_model_id=feature_model_id, user_id=user_id
    ).first()
    if review:
        review.value = value
    else:
        review = FeatureModelReview(
            feature_model_id=feature_model_id, user_id=user_id, value=value
        )
    ...
    total_likes = (
        db.session.query(db.func.sum(FeatureModelReview.value))
