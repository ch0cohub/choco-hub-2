...
def setup_deposition(test_client):
    with test_client.application.app_context():
        deposition = Deposition(
            id=1, dep_metadata={"title": "Test Deposition"}, status="draft", doi=None
        )
...
def test_create_new_deposition(fakenodo_service, test_client):
    with test_client.application.app_context():
        ...
        with patch.object(
            fakenodo_service.deposition_repository,
            "create_new_deposition",
            return_value=mock_deposition,
        ) as mock_create:
...
def test_upload_file(fakenodo_service, test_client):
    with test_client.application.app_context():
        ...
        with patch("os.path.getsize", return_value=100):
            result = fakenodo_service.upload_file(
                mock_dataset, 1, mock_feature_model, user=mock_user
            )
            assert result["id"] == 1
...
def test_get_doi(fakenodo_service, test_client):
    with test_client.application.app_context():
        ...
        with patch.object(
            fakenodo_service, "get_deposition", return_value={"doi": "fakenodo.doi.1"}
        ):
