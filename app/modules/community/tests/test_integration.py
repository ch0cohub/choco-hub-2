...
def setup_test_data():
    ...
    community = community_service.create(
        name="Test Community", description="This is a test community.", owner_id=user.id
    )

    # Crear otro usuario no propietario
    other_user = User(email="otheruser@example.com", password="otherpassword")
    profile_other = UserProfile(
        name="Other", surname="User", is_verified=True, user=other_user
    )
...
def test_get_community(test_client):
    ...
    assert (
        b"This is a test community." in response.data
    ), "Community description was not displayed."
...
def test_join_community(test_client):
    ...
    with test_client.application.app_context():
        ...
        assert (
            other_user not in community.members
        ), "User is already a member of the community."

    response = test_client.post("/community/join", json={"community_id": community.id})
...
def test_leave_community(test_client):
    ...
    response = test_client.post("/community/leave", json={"community_id": community.id})
    assert response.status_code == 200, "Failed to leave the community."
    assert (
        b"You have successfully left the community" in response.data
    ...
    with test_client.application.app_context():
        ...
        assert (
            other_user not in community.members
        ), "User was not removed from the community."
...
def test_edit_community(test_client):
    ...
    assert (
        b"Updated Community" in response.data
    ), "Updated community name is not displayed."

    with test_client.application.app_context():
        updated_community = Community.query.get(community.id)
        assert (
            updated_community.name == "Updated Community"
        ), "Community name was not updated."
...
def test_delete_community(test_client):
    ...
    response = test_client.post(
        f"/community/delete/{community.id}", follow_redirects=True
    )
