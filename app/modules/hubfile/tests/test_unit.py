...
def test_create_hubfile(test_client):
    with test_client.application.app_context():
        ...
        dsmetadata = DSMetaData(id=1, title="Test Dataset", description="Test Description", publication_type=PublicationType.JOURNAL_ARTICLE)
        ...
        fmmetadata = FMMetaData(id=1, uvl_filename="file1.uvl", title="Feature Model 1", description="Description for feature model 1", publication_type=PublicationType.SOFTWARE_DOCUMENTATION)
        db.session.add(fmmetadata)
        db.session.commit()
        # Create a new DSMetaData and DataSet to use as a foreign key before creating a new FeatureModel
        feature_model = FeatureModel(id=99, data_set_id=dataset.id, fm_meta_data_id=fmmetadata.id)
...
def test_delete_hubfile(test_client):
    with test_client.application.app_context():
        ...
        # New DSMetaData y DataSet for testing
        dsmetadata = DSMetaData(id=2, title="Test Dataset", description="Test Description", publication_type=PublicationType.JOURNAL_ARTICLE)
        db.session.add(dsmetadata)
        db.session.commit()
        ...
        db.session.commit()
        # New FMMetaData for testing
        fmmetadata = FMMetaData(id=2, uvl_filename="file1.uvl", title="Feature Model 1", description="Description for feature model 1", publication_type=PublicationType.SOFTWARE_DOCUMENTATION)
        db.session.add(fmmetadata)
        db.session.commit()
        # Create a new DSMetaData and DataSet to use as a foreign key before creating a new FeatureModel
        feature_model = FeatureModel(id=100, data_set_id=dataset.id, fm_meta_data_id=fmmetadata.id)
        db.session.add(feature_model)
        db.session.commit()
        # Create a new hubfile
        new_hubfile = hubfile_service.create(
